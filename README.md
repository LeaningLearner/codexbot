...
安装插件后，在 Codex 的 `/hooks` 页面中确认 `codexbot` 的生命周期 Hook 已被信任：
...
1. 重启 Codex。
2. 在 Codex 中打开 `/hooks`，检查并信任 `codexbot` 的生命周期 Hook。
...
主动消息按每分钟 18 条节流，低于 QQ 官方单关系 20 qpm 的限制。网络错误会重试；QQ 拒收、关闭主动消息、内容违规等永久错误不会循环发送，最近回复仍可用 `/last` 被动读取。QQ 仍然是只读通知端，不能在 QQ 中批准或拒绝 Codex 操作。
